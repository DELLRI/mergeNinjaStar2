...
import logic_artifact   # ⭐ 변경됨
import storage_artifact  # ⭐ 변경됨

class ArtifactCalculatorTab:
    def __init__(self, parent, root):
        self.parent = parent # ⭐ 유물 계산기가 그려질 첫 번째 탭 프레임
        self.root = root     # ⭐ 메인 윈도우 (엔터키 포커스 제어용)
    ...
    def setup_ui(self):
        # ⭐ 모든 UI 요소의 부모를 self.root에서 self.parent로 변경
        tk.Label(self.parent, text="고급 유물 계산기", font=("Arial", 20, "bold"), anchor="w").pack(fill="x", padx=10, pady=10)
        ...
        # 기존 manage_frame 내부의 버튼들을 정리합니다.
        manage_frame = tk.LabelFrame(right_frame, text="항목 관리", font=("Arial", 10, "bold"), padx=10, pady=10, bg="#F8F9FA")
        ...
        # ⭐ 버튼 명칭 변경 및 기능 변경
        tk.Button(manage_frame, text="추천레벨 초기화 🔄", bg="#FFF3CD", command=self.reset_recommend_levels, width=16).pack(side="left", padx=10)

        # 테스트 데이터 초기화는 헷갈리지 않게 버튼 하나를 더 만들어 아래에 배치하거나 별도 메뉴로 뺍니다.
        tk.Button(manage_frame, text="전체 데이터 초기화 ⚠️", bg="#FFCCCB", command=self.reset_data_to_test, width=16).pack(side="left", padx=10)
    ...
    def render_output_zone(self):
        ...
        for i, item in enumerate(sorted_categories):
            ...
            tk.Label(self.table_frame, text=art_name, relief="solid", borderwidth=1, bg="white").grid(row=row, column=0, sticky="nsew")

            if cat not in self.level_vars: self.level_vars[cat] = tk.StringVar(value="0")
            ...
            self.level_vars[cat].trace_add("write", lambda *args, c=cat: self.recalc_atk_for_manual_level(c))
            ...
            self.result_ui[cat] = {"level_var": self.level_vars[cat], "atk_lbl": atk_lbl, "coef_lbl": coef_lbl, "eff_lbl": eff_lbl}
    ...
    def update_summary_labels(self):
        ...
        self.lbl_remain_dia.config(fg="red" if remain_dia < 0 else "#1976D2")
    ...
    def on_apply_result(self):
        ...
        if total_cost == 0:
            return messagebox.showinfo("안내", "소모된 다이아가 없거나 이미 적용된 상태입니다.")
        if remain_dia < 0:
            return messagebox.showwarning("경고", "잔여 다이아가 부족하여 결과를 적용할 수 없습니다.")
        ...
        if messagebox.askyesno("결과 적용 확정", confirm_msg):
            ...
            for item in self.categories:
                cat = item["name"]
                if cat in self.current_results and self.current_results[cat].get("dia_cost", 0) > 0:
            ...
            self.lbl_remain_dia.config(text=f"계산 후 잔여 다이아: {int(remain_dia):,} 💎", fg="#1976D2")
    ...
    def recalc_atk_for_manual_level(self, cat):
        if cat not in self.current_results: return
        try:
            ...
            self.current_results[cat]["dia_cost"] = max(0, new_cost - base_cost)
    ...
    def load_initial_data(self):
        self.saved_data = storage_artifact.load_data()
        self.categories = self.saved_data.get("categories", [])
        self.formulas = self.saved_data.get("formulas", storage_artifact.DEFAULT_FORMULAS)
        self.saved_levels = {}
        ...
        for item in self.categories:
            ...
            if cat not in self.entry_vars: self.entry_vars[cat] = tk.StringVar(value="0")
            if cat not in self.daily_vars: self.daily_vars[cat] = tk.StringVar(value=str(item.get("default_daily", 0)))

            if f"{cat}_입력" in self.saved_data: self.entry_vars[cat].set(f"{self.saved_data[f'{cat}_입력']:,}")
            if f"{cat}_일일" in self.saved_data: self.daily_vars[cat].set(f"{self.saved_data[f'{cat}_일일']:,}")
        if "다이아" in self.saved_data: self.diamond_var.set(f"{self.saved_data['다이아']:,}")
    ...
    def save_current_data(self):
        ...
        data = {
            ...
            "다이아": self.get_int_value(self.diamond_var), "results": saved_results_list
        }
        for item in self.categories:
            cat = item["name"]
        ...
        storage_artifact.save_data(data)

    def reset_data_to_test(self):
        if messagebox.askyesno("초기화 확인", "테스트용 클린 데이터로 리셋됩니다. 진행하시겠습니까?"):
            storage_artifact.reset_to_test_data()
            self.entry_vars.clear()
            self.daily_vars.clear()
            self.level_vars.clear()
            self.current_results.clear()
            self.saved_levels.clear()
            self.load_initial_data()
            self.render_input_zone()
            self.render_output_zone()
            self.on_calculate()
    ...
    def add_category(self):
        name = simpledialog.askstring("항목 추가", "신규 콘텐츠 이름을 입력하세요:")
        if not name: return
        art_name = simpledialog.askstring("유물 이름", f"'{name}'의 유물 이름을 입력하세요:")
        coef_str = simpledialog.askstring("계수 입력", "계수를 입력하세요 (숫자만):")

        self.categories.append({"name": name, "artifact_name": art_name or name, "coef": int(coef_str) if coef_str and coef_str.isdigit() else 10, "default_daily": 0, "out_idx": len(self.categories) + 1})
    ...
    def delete_category(self):
        ...
        def do_delete():
            ...
            if messagebox.askyesno("삭제 확인", "정말 삭제하시겠습니까?", parent=pop):
                ...
                for d in [self.entry_vars, self.daily_vars, self.level_vars, self.current_results, self.saved_levels]:
    ...
    def on_attendance(self):
        ...
        updated = logic_artifact.do_attendance(entries, dailies)
        for item in self.categories: self.entry_vars[item["name"]].set(f"{updated[item['name']]:,}")
        self.on_calculate()
        messagebox.showinfo("완료", "출석체크 완료!")

    def on_calculate(self):
        entries = {item["name"]: self.get_int_value(self.entry_vars[item["name"]]) for item in self.categories}
        diamonds = self.get_int_value(self.diamond_var)
        results = logic_artifact.calculate_artifacts(entries, diamonds, self.categories, self.formulas, self.saved_levels)
        for res in results: self.current_results[res["name"]] = res
        self.sync_ui_from_current_results()
        self.save_current_data()

    def reset_recommend_levels(self):
        """유물 계산기의 추천레벨만 0으로 초기화합니다."""
        if messagebox.askyesno("초기화 확인", "모든 유물의 추천레벨을 0으로 초기화하시겠습니까?"):
            for name in self.level_vars:
                ...
                if name in self.current_results:
                    ...
                    self.current_results[name]["dia_cost"] = 0
                if name in self.result_ui:
                    self.result_ui[name]["atk_lbl"].config(text="0.0000")

            self.update_summary_labels()
            messagebox.showinfo("완료", "추천레벨이 초기화되었습니다.")

    def reset_data_to_test(self):
        """기존의 테스트 데이터 복구/초기화 기능 (전체 초기화용)"""
        if messagebox.askyesno("전체 데이터 초기화", "모든 저장 데이터와 설정이 초기화됩니다. 진행하시겠습니까?"):
            storage_artifact.reset_to_test_data()
            self.entry_vars.clear()
            self.daily_vars.clear()
            self.level_vars.clear()
            ...
            self.render_output_zone()
            self.on_calculate()
            messagebox.showinfo("성공", "전체 데이터가 초기화되었습니다!")